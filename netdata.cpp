...
void NetData::apply( ) {
	...
	if( /*std::abs( view_delta.x ) <= 0.125f &&
		std::abs( view_delta.y ) <= 0.125f &&*/
		std::abs( view_delta.z ) <= 0.125f )
        g_cl.m_local->m_vecViewOffset( ).z = fminf( fmaxf( data->m_view_offset.z, 46.f ), 64.f );
}
