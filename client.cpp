...
void Client::StartMove( CUserCmd* cmd ) {
	...
	m_latency += g_csgo.m_net->GetLatency( INetChannel::FLOW_INCOMING );
	...
	// did fake walk.
	static bool did_fake_walk = false;

	// make sure if u are in fake walking..
	if ( m_fake_walk ) {
		if ( !did_fake_walk ) {
			g_movement.QuickStop( );

			if ( m_local->m_vecVelocity( ).length( ) < 0.1f )
				did_fake_walk = true;
		}
	}
...
void Client::DoMove( ) {
	...
	if( m_weapon ) {
		...
		/*auto is_special_weapon = ( m_weapon_id == 9
								   ...
								   || m_weapon_id == 38
								   || m_weapon_id == 40 );
		if ( m_weapon_id && m_weapon_info ) {
			if ( m_flags & FL_DUCKING ) {
				if ( is_special_weapon )
					m_calc_inaccuracy = m_weapon_info->m_inaccuracy_crouch_alt;
				else
					m_calc_inaccuracy = m_weapon_info->m_inaccuracy_crouch;
			}
			else if ( is_special_weapon )
				m_calc_inaccuracy = m_weapon_info->m_inaccuracy_stand_alt;
			else
				m_calc_inaccuracy = m_weapon_info->m_inaccuracy_stand;
		}*/
...
void Client::UpdateAnimations( ) {
	...
	if ( !g_csgo.m_cl->m_choked_commands
		 || state->m_time != old_time ) {
		old_time = state->m_time;

		float curtime = g_csgo.m_globals->m_curtime;
		float frametime = g_csgo.m_globals->m_frametime;

		g_csgo.m_globals->m_curtime = g_cl.m_local->m_flOldSimulationTime( ) * g_csgo.m_globals->m_interval;
		...
		// s/o onetap
		auto sixth_overlay = &g_cl.m_local->m_AnimOverlay( )[ 6 ];

		if ( sixth_overlay && g_cl.m_fake_walk ) {
			sixth_overlay->m_weight = sixth_overlay->m_playback_rate = 0.0f;
			g_cl.m_local->m_flPoseParameter( )[ 3 ] = 0.f;
			g_cl.m_local->m_flPoseParameter( )[ 4 ] = 0.f;
		}
