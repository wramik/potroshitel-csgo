...
void HVH::SendPacket( ) {
	...
	if( g_menu.main.antiaim.lag_enable.get( ) && !g_csgo.m_gamerules->m_bFreezePeriod( ) && !( g_cl.m_flags & FL_FROZEN ) ) {
		...
		if( active ) {
			...
			// adaptive ( thanks polak ).
			else if ( mode == 4 ) {
				// get lean weight.
				float lean = g_cl.m_local->m_AnimOverlay( )[ 12 ].m_weight;

				int adaptive_ticks = limit - 1;
				float extrapolated_speed = g_cl.m_local->m_vecVelocity( ).length_2d( ) * g_csgo.m_globals->m_interval;

				if ( extrapolated_speed <= 68.0 ) {
					if ( ( ( adaptive_ticks - 2 ) * extrapolated_speed ) > 68.0 ) {
						++adaptive_ticks;
						//extrapolated_speed -= 68.0 / ( float )adaptive_ticks - 16;
					}

					// move ticks.
					// formula ->
					// lean >= 0.15 if in air or fast moving or chaing wpn increase.
					adaptive_ticks += 5;
					if ( adaptive_ticks > 11
						 || lean >= 0.15f )
						adaptive_ticks += limit - 2;
				...
				//while ( ( wish_ticks * extrapolated_speed ) <= 68.0 ) {
				//	if ( ( ( adaptive_ticks - 1 ) * extrapolated_speed ) > 68.0 ) {
				//		++wish_ticks;
				//		//break;
				//	}
				//	if ( ( adaptive_ticks * extrapolated_speed ) > 68.0 ) {
				//		wish_ticks += 2;
				//		//break;
				//	}
				...
				//	if ( wish_ticks > limit && adaptive_ticks <= 16 )
				//		*g_cl.m_packet = false;
				//}
				/*const int units_per_tick = static_cast< int >( game::TICKS_TO_TIME( g_cl.m_local->m_vecVelocity( ).length( ) ) );

				if ( delta >= 4096.f
					 || units_per_tick >= 68
					 && ( delta - ( float )units_per_tick ) - 1 <= 15 )
					*g_cl.m_packet = false;*/
				/*int wish_ticks{};
				int adaptive_ticks{};
				...
				if ( ( adaptive_ticks - 1 ) * units_per_tick > 68 )
				{
					++wish_ticks;
				LABEL_64:
					should_choked = g_csgo.m_cl->m_choked_commands < wish_ticks;
				}
				else if ( adaptive_ticks * units_per_tick > 68 )
					should_choked = g_csgo.m_cl->m_choked_commands < wish_ticks + 2;
				else if ( ( adaptive_ticks + 1 ) * units_per_tick > 68 )
					should_choked = g_csgo.m_cl->m_choked_commands < wish_ticks + 3;
				else {
					...
					should_choked = g_csgo.m_cl->m_choked_commands < wish_ticks + 4;
				}

				int choke_amnt{};
				choke_amnt = wish_ticks;*/
			}
