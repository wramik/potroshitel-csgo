...
void Aimbot::find( ) {
	...
	if ( best.player && best.record ) {
		...
		// set autostop shit.
		m_stop = g_menu.main.movement.autostop.get( )/*( int )g_menu.main.movement.autostop.get( ) > 0*//*!( g_cl.m_buttons & IN_JUMP )*/;
...
bool Aimbot::CheckHitchance( Player *player, const ang_t &angle, bool trace_think, int hitbox ) {
	...
	// get final think.
	float final_think = inaccuracy + spread;
	...
	for ( int i{ }; i <= SEED_MAX; ++i ) {
		...
		// debug shit.
		if ( TESTING_GANG )
			g_csgo.m_debug_overlay->AddLineOverlay( start, end, 0, 255, 0, false, 2.f * g_csgo.m_globals->m_interval );

		// check if we hit a valid player / hitgroup on the player and increment total hits.
		if ( tr.m_entity == player && game::IsValidHitgroup( tr.m_hitgroup ) ) {
			if ( trace_think )
				total_hits++;
			else if ( tr.m_hitbox == hitbox )
				total_hits++;
			...
			// captache accuracy boost.
			if ( g_menu.main.aimbot.accuracy_boost.get( ) ) {
				if ( g_menu.main.aimbot.accuracy_boost_amount.get( ) <= final_think && g_cl.CanFireWeapon( ) )
					return true;
			}
		}
	}
	...
	if ( attack ) {
		...
		*g_cl.m_packet = true;
		...
		if ( m_target ) {
			...
			if ( m_record && !m_record->m_broke_lc /*|| ( g_csgo.m_cl->m_server_tick - m_record->m_tick ) > 1*/ )
			...
			g_visuals.DrawHitboxMatrix( m_record, colors::red, 4.f );
		}
