...
public:
	...
	__forceinline void ModifyEyePosition(CCSGOPlayerAnimState *state, vec3_t *pos) {
		...
		if (state->m_player &&
			...
			auto head_bone = g_csgo.LookupBone( state->m_player, "head_0" );

			if ( head_bone != -1 ) {
				vec3_t head_pos(
					state->m_player->m_BoneCache( ).m_pCachedBones[ head_bone ][ 0 ][ 3 ],
					state->m_player->m_BoneCache( ).m_pCachedBones[ head_bone ][ 1 ][ 3 ],
					state->m_player->m_BoneCache( ).m_pCachedBones[ head_bone ][ 2 ][ 3 ] );
				...
				if ( ( *pos ).z > head_pos.z ) {
					...
					float delta = ( *pos ).z - head_pos.z;

					float some_offset = ( delta - 4.f ) / 6.f;
					if ( some_offset >= 0.f )
						some_factor = std::fminf( some_offset, 1.f );

					( *pos ).z += ( ( head_pos.z - ( *pos ).z ) * ( ( ( some_factor * some_factor ) * 3.f ) - ( ( ( some_factor * some_factor ) * 2.f ) * some_factor ) ) );
				}
			}
		}
	}
	...
	__forceinline vec3_t GetShootPosition() {
		if ( this == g_cl.m_local ) {
			...
			util::get_method< void( __thiscall* )( decltype( this ), vec3_t& ) >( this, 277 )( this, pos );

			if ( m_vecViewOffset( ).z < 46.1f ) {
				pos.z -= m_vecViewOffset( ).z;
				pos.z += 46.f;
			}
